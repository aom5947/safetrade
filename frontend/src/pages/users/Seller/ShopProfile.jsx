...
import { Search, Store, Share2, MessageCircle, Star, Package } from 'lucide-react';
...
export default function ShopProfile() {
  ...
  // ✅ helper numbers ป้องกัน toFixed พัง
  const averageRatingNumber = Number(reviewStats?.average_rating || 0);
  const totalReviewsNumber = Number(reviewStats?.total_reviews || 0);
  ...
  useEffect(() => {
    const fetchSellerProfile = async () => {
      try {
        ...
        // backend ส่ง { success, data } (ไม่มี data.seller)
  ...
  // ✅ ดึงข้อมูลสินค้าตาม tab ที่เลือก
  useEffect(() => {
    const fetchListings = async () => {
      try {
        ...
        // backend ส่ง { success, data: [...], pagination: {...} }
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        ...
        // backend ส่ง { success, data: [...], sellerRating: { average, count }, pagination: {...} }
        if (response.data.success) {
          setReviews(response.data.data || []);

          if (response.data.sellerRating) {
            setReviewStats({
              average_rating: Number(response.data.sellerRating.average) || 0,
              total_reviews: Number(response.data.sellerRating.count) || 0,
            });
          }
  ...
  const currentReviewPage =
    Math.floor(reviewPagination.offset / reviewPagination.limit) + 1;
                ...
                <div>
                  ...
                  <div className="flex items-center gap-6 text-sm text-gray-600">
                    <div className="flex items-center gap-1">
                      ...
                      <span>
                        {averageRatingNumber.toFixed(1)} ({totalReviewsNumber}{' '}
                        รีวิว)
                      </span>
                    </div>
                    <div className="flex items-center gap-1">
                      ...
                      <div className="flex items-center justify-center mt-1">
                        {[...Array(5)].map((_, i) => (
                          <Star
                            key={i}
                            className={`w-4 h-4 ${
                              i < Math.round(averageRatingNumber)
