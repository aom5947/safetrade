...
const ListingDetailModal = ({ isOpen, onClose, listing }) => {
  ...
  const [localImages, setLocalImages] = useState(listing?.images || []);
  ...
  // Normalize images: if string, convert to object with dummy id
  const images = localImages.map((img, idx) =>
    typeof img === 'string' ? { image_id: null, image_url: img } : img
  );
          ...
          {images.length > 0 && (
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                {/* Delete Image Button */}
                {(currentImage.image_id || currentImage.image_id === null) && (
                  <Button
                    variant="destructive"
                    size="sm"
                ...
                <div className="flex gap-2 overflow-x-auto">
                  {images.map((img, index) => (
                    <button
                      ...
                      className={`flex-shrink-0 w-20 h-20 rounded border-2 transition-all ${currentImageIndex === index
                          ? 'border-primary'
